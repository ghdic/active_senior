...
public class BbsDAO {
    ...
    public ArrayList<Bbs> getPostList(int pageNumber, int require) {
        String SQL = "select bbsID, bbsTitle, userID, bbsDate, bbsView, bbsRecommend from bbs where bbsAvailable = 1 order by bbsID desc limit ?, ?";
        ...
        try {
            ...
            while(rs.next()) {
                ...
                bbs.setBbsView(rs.getInt(5));
                bbs.setBbsRecommend(rs.getInt(6));
                list.add(bbs);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public boolean nextPage(int pageNumber, int require) {
        String SQL = "select bbsID from bbs where bbsAvailable = 1 order by bbsID desc limit ?, 1";
        try {
            PreparedStatement pstmt = conn.prepareStatement(SQL);
    ...
    public Bbs getPost(int bbsID) {
        ...
        try {
            ...
            if(rs.next()) {
                ...
                bbs.setBbsRecommend(rs.getInt(8));
                return bbs;
            }
        } catch (Exception e) {
            e.printStackTrace();
